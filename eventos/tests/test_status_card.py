...
class EventoStatusCardTests(TestCase):
    def test_evento_sem_documentos_fica_rascunho(self):
        evento = Evento.objects.create(titulo="Evento vazio")
    ...
    def test_oficio_pendente_bloqueia_mesmo_com_convite(self):
        evento = Evento.objects.create(titulo="Evento com ofício pendente")
        Oficio.objects.create(evento=evento, status=Oficio.STATUS_RASCUNHO, assunto="x")
    ...
    def test_convite_sem_oficio_pendente_fica_pronto(self):
        evento = Evento.objects.create(titulo="Evento só com convite")
        EventoAnexo.objects.create(
            evento=evento, tipo=EventoAnexo.TIPO_CONVITE,
            arquivo=ContentFile(b"conteudo", name="convite.pdf"),
        )
    ...
    def test_oficio_gerado_com_convite_mostra_contagem_ate_saida(self):
        ...
        evento = Evento.objects.create(titulo="Evento pronto com saída futura")
        saida = timezone.now() + timedelta(days=5)
        roteiro = Roteiro.objects.create(saida_dt=saida, evento=evento)
        oficio = Oficio.objects.create(evento=evento, status=Oficio.STATUS_GERADO, assunto="x")
    ...
    def test_evento_cancelado_mostra_cancelado(self):
        evento = Evento.objects.create(titulo="Evento cancelado", status=Evento.STATUS_CANCELADO)
        card = apresentar_evento_list_card(_evento_com_prefetch(evento.pk))
