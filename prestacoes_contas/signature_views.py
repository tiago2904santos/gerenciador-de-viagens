...
def _fallback_da_prestacao(prestacao) -> str:
    ...
    if ps is not None:
        return reverse("prestacoes_contas:consolidado_servidor", args=[ps.pk])
    return reverse("prestacoes_contas:index")
...
def assinatura_rt_gerar(request, ps_pk):
    ...
    next_url = voltar_para(
        request, reverse("prestacoes_contas:consolidado_servidor", args=[ps.pk])
    )
    try:
