...
- [ ] `BE-14` 48 sites de persistência em view, sem transação — **eram 36**, mais 4 por método de
      ...
      divergência estava registrada com o ID de outro defeito — é decisão de produto). Faltam os
      anexos e o diário — e a dos anexos **não é pôr `atomic`**: precisa de `transaction.on_commit`,
      senão inverte o órfão do `BE-07`
