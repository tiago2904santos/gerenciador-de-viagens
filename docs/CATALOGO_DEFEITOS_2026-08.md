...
### NOVO-20260813-154133-b0d4495f5dec ✅ RESOLVIDO · `NOVO` Merge gravou conflitos em páginas e planos · QA/HT · 0,25 d

O merge `0434421b` registrou literalmente `<<<<<<<`, `=======` e `>>>>>>>` em cinco arquivos:
as listas de Ofícios e Eventos, o catálogo de defeitos, o plano mestre e o plano de reconstrução
frontend. As duas páginas quebravam antes de montar o shell com `TemplateSyntaxError` por repetirem
o bloco `shell_css`; por isso o sidebar nem chegava a existir nessas rotas. Os três documentos
também ficaram duplicados e contraditórios.

**Correção.** Os cinco arquivos voltaram ao conteúdo vigente de `9f64b661`, preservando o perfil
...
`planos_trabalho/tests/test_wizard_v2.py`). Sete componentes novos entraram no v2 para cobrir o que
faltava — `pending_card`, `choice_card`, `choice_grid`, `number_stepper`, `live_list`,
`efetivo_row` e `document_preview` —, todos publicados na galeria. As duas folhas de página do app
