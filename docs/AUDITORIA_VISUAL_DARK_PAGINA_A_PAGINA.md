...
| NOVO-21 | ✅ | **Lista de Servidores: toolbar só com paginação** — lado esquerdo vazio. **Corrigido:** abas `cv-list-tabs` com Todos + 3 cargos mais usados (`?cargo=`). | `servidores_index` · `list_tabs` · `selectors.cargos_mais_frequentes_servidores` |
| NOVO-22 | ✅ | **Lista de Viaturas: toolbar só com paginação** — sem atalho à unidade da config nem combustíveis. **Corrigido:** abas Todos + unidade das configurações + 3 combustíveis mais usados. | `viaturas_index` · `selectors.combustiveis_mais_frequentes_viaturas` |
| NOVO-23 | ✅ | **`.cv-floating-action--stacked` nunca valeu** — o modificador morava em `page-shell.css`, mas `cv-buttons.css` redeclara `.cv-floating-action { bottom }` e vem depois na ordem do bundle: mesma especificidade, e a base ganhava. Toda página com duas ações flutuantes (o `back_url` de `list_page_standard`) empilhava uma **em cima** da outra. Medido no navegador: as duas caixas em `y=932`. **Corrigido:** modificador movido para `cv-buttons.css`, depois da base, com o par mobile; após a correção, `y=932` e `y=876`. | `cv-buttons.css` · `page-shell.css` |
| NOVO-24 | ✅ | **Administração de Usuários/Áreas era um formulário com listas no fim** — `variant="split"` (feito para campo de formulário) aplicado a duas tabelas deixava uma coluna morta de ~200×600px com o título boiando no meio vertical; três formulários independentes empilhados num card só, cada um com seu botão primário, empurravam os dados para o fim de uma rolagem longa; e a tabela de vínculos repetia a mesma pessoa em N linhas. **Corrigido:** duas páginas no padrão `list_page_standard` (`/usuarios/` e `/usuarios/areas/`), alternadas por `cv-segment-toggle`; uma linha por pessoa com as áreas como fatos da própria linha; as três criações viraram páginas de formulário próprias. | `usuarios/*` · `templates/usuarios/*` |
...
| Administração / Usuários | `usuarios/index.html` | ✅ lista `list_page_standard` + `cv-record-row`, uma linha por pessoa; alternador `cv-segment-toggle` para Áreas (NOVO-24). O `cv-metric-grid` do D-55 saiu junto com a página de formulário — os números viraram os contadores do alternador. |
| Administração / Áreas | `usuarios/areas/index.html` | ✅ lista `list_page_standard` + `cv-record-row` (NOVO-24). Sem `avatar`: a inicial de uma sigla é a própria sigla. |
| Administração / criar (3) | `usuarios/form.html` | ✅ casco único das três criações (usuário, área, vínculo), no mesmo padrão de `cadastros/*/form.html` (NOVO-24). |
