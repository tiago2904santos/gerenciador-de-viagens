...
class PdfRenderer(BaseDocumentRenderer):
    ...
    def render(self, request: RenderRequest) -> RenderResult:
        if request.formato != self.formato:
            raise RendererUnavailableError("Renderer PDF recebeu formato incompatível.")
        if not self._adapter:
            raise RendererUnavailableError("Renderer PDF sem adapter configurado.")
        content = self._adapter(request)
        return RenderResult(content=content, content_type="application/pdf")
